...
const GAME_CONFIG: Dictionary = {
	...
	"danger_zone_count": {"label": "Nombre de zones dangereuses", "description": "Nombre de zones dangereuses statiques générées depuis la seed. Zéro désactive strictement la mécanique.", "scope": "globale", "category": "environnement", "type": "int", "min": 0, "max": 100, "default": 0, "dynamic": false, "live_editable": false},
	"danger_zone_radius": {"label": "Rayon des zones dangereuses", "description": "Rayon, en mètres, de chaque zone dangereuse circulaire.", "scope": "globale", "category": "environnement", "type": "float", "min": 0.1, "max": 80.0, "default": 8.0, "dynamic": false, "live_editable": false},
	"danger_hunger_cost_rate": {"label": "Coût de faim du danger", "description": "Perte de faim additionnelle par seconde simulée pendant une exposition au danger.", "scope": "globale", "category": "environnement", "type": "float", "min": 0.0, "max": 20.0, "default": 1.0, "dynamic": false, "live_editable": false},
	"danger_zone_visible": {"label": "Zones dangereuses visibles", "description": "Rend les zones dangereuses perceptibles par la vision générique des agents.", "scope": "globale", "category": "environnement", "type": "bool", "default": true, "dynamic": false, "live_editable": false},
	"danger_zone_safety_radius": {"label": "Rayon de sécurité danger", "description": "Distance minimale entre une zone dangereuse et un spawn ou un roncier lors du placement aléatoire. En mode approche_roncier, elle ne protège que les spawns afin que la zone puisse garder une ressource malgré les ronciers voisins.", "scope": "globale", "category": "environnement", "type": "float", "min": 0.0, "max": 100.0, "default": 12.0, "dynamic": false, "live_editable": false},
	"danger_zone_placement_mode": {"label": "Mode de placement danger", "description": "aleatoire répartit les zones hors des ronciers ; approche_roncier les place sur le segment entre un spawn et des ronciers déterministes.", "scope": "globale", "category": "environnement", "type": "enum", "options": ["aleatoire", "approche_roncier"], "default": "aleatoire", "dynamic": false, "live_editable": false},
	"danger_zone_approach_spawn_index": {"label": "Spawn d'approche danger", "description": "Indice du spawn source utilisé par approche_roncier : 0=Rouge, 1=Bleu, 2=Vert, 3=Jaune.", "scope": "globale", "category": "environnement", "type": "int", "min": 0, "max": 3, "default": 0, "dynamic": false, "live_editable": false},
	"danger_zone_approach_clearance": {"label": "Marge d'approche danger", "description": "Écart entre le bord d'une zone approche_roncier et son roncier source, en mètres.", "scope": "globale", "category": "environnement", "type": "float", "min": 0.0, "max": 100.0, "default": 2.0, "dynamic": false, "live_editable": false},
...
const CHARACTER: Dictionary = {
	...
	"fixed_policy_s2": {"label": "Politique fixe S2 (souvenir)", "description": "Action tenue par le décideur politique_fixe quand aucun roncier n'est visible mais qu'un souvenir de roncier est utilisable : viser le souvenir le plus proche, viser le souvenir le plus estompé, ou errer. Ignoré par les autres décideurs.", "scope": "individuelle", "category": "décision", "type": "enum", "options": ["ronce_memorisee", "souvenir_ancien", "errance"], "default": "ronce_memorisee", "dynamic": false, "live_editable": false},
	"fixed_policy_s3": {"label": "Politique fixe S3 (inconnu)", "description": "Action tenue par le décideur politique_fixe quand ni roncier visible ni souvenir utilisable : errer, maintenir le cap courant, faire demi-tour, viser la zone inconnue la plus proche, ou viser la zone déjà visitée la plus proche. Ignoré par les autres décideurs.", "scope": "individuelle", "category": "décision", "type": "enum", "options": ["errance", "cap_maintenu", "demi_tour", "zone_inconnue", "zone_connue"], "default": "errance", "dynamic": false, "live_editable": false},
	"fixed_policy_danger": {"label": "Politique fixe danger", "description": "Réponse du décideur politique_fixe à un danger physiquement subi ou visible : l'ignorer (politique alimentaire inchangée), s'en éloigner, le viser (contrôle négatif), ou tirer eviter/viser au hasard à chaque décision (plancher sans réponse structurée, Phase 3 roadmap_environnement_apprenable_v3). Surcouche à fixed_policy_s1/s2/s3, sans effet hors danger. Ignoré par les autres décideurs.", "scope": "individuelle", "category": "décision", "type": "enum", "options": ["ignorer", "eviter", "viser", "aleatoire"], "default": "ignorer", "dynamic": false, "live_editable": false},
	"danger_reaction_range": {"label": "Distance de réaction au danger", "description": "Distance maximale à laquelle une zone dangereuse seulement visible (non physiquement subie) déclenche la surcouche fixed_policy_danger. Un danger visible au-delà de cette distance n'interrompt pas la politique alimentaire ; un danger physiquement subi déclenche toujours la réponse, quelle que soit cette valeur. Distinct de vision_range (perception) : réduit l'agressivité de la surcouche sans réduire ce que l'agent perçoit. Défaut au maximum de vision_range (aucun filtrage tant que la valeur n'est pas réduite explicitement). Ignoré par les autres décideurs.", "scope": "individuelle", "category": "décision", "type": "float", "min": 0.0, "max": 250.0, "default": 250.0, "dynamic": false, "live_editable": false},
	...
	"survival_cost_rate": {"label": "Coût de survie (récompense)", "description": "Pénalité de récompense par seconde de fenêtre d'engagement du décideur adaptatif : la cible d'une fenêtre sans cueillette vaut -survival_cost_rate x durée. Rend explicite le décalage constant de la récompense.", "scope": "individuelle", "category": "décision", "type": "float", "min": 0.0, "max": 5.0, "default": 0.05, "dynamic": false, "live_editable": false},
	"td_discount_gamma": {"label": "Facteur d'actualisation TD", "description": "Facteur gamma de l'amorçage par différence temporelle du décideur adaptatif : la cible d'une fenêtre vaut reward + gamma x meilleur score de l'état suivant. 0 supprime la propagation du crédit.", "scope": "individuelle", "category": "décision", "type": "float", "min": 0.0, "max": 1.0, "default": 0.9, "dynamic": false, "live_editable": false},

...
**Au lancement** (`python run.py`) : la fenêtre s'ouvre directement en vue d'ensemble
fixe (caméra FREE, position (0, 60, 80), regard plongeant à -30°), les 4 panneaux
personnage sont déjà affichés en permanence dans les coins (aucun clic requis pour les
voir), la souris est immédiatement capturée (`MOUSE_MODE_CAPTURED` dans
`_ready()` de `free_camera.gd`) — l'utilisateur ne peut donc pas cliquer sur un bouton
UI tant qu'il n'a pas appuyé sur Échap pour la libérer. Ce n'est indiqué nulle part à
l'écran : premier point de friction potentiel pour quelqu'un qui découvre le projet.

**Actions disponibles et découvrabilité** : aucun tutoriel ni texte d'aide n'existe dans
le code. Tout repose sur la convention FPS classique (ZQSD + souris + Maj pour courir,
E/C pour monter/descendre) et sur l'exploration des boutons du menu bas, qui sont
explicites par leur libellé ("Relancer", "Large", "Données du jeu", noms colorés des
personnages) mais dont l'effet en cascade (cycle de caméra à 3 clics, limite de regard
en vue FPS) n'est pas annoncé avant d'être testé.

**Retours visuels donnés à l'utilisateur** :
- Barre de faim (rouge) qui descend en continu sous chaque panneau — seul indicateur de
  l'urgence de la situation d'un personnage.
- Compteurs "Mûres ramassées/mangées (total)" et "Ronciers mémorisés" mis à jour en
  direct, visibles même après la mort (contrairement à la section "État", masquée en
  permanence côté code).
- Label "DEAD" en rouge qui remplace le contenu vivant du panneau à la mort, synchronisé
  avec la bascule physique du personnage (tween 0.6s).
- Ronce : changement de matériau (violet plein → gris-vert vide) au fur et à mesure de
  la cueillette, seul indice visuel de son stock restant (pas de compteur numérique
  affiché sur la ronce elle-même).
- Molette de souris en mode FOLLOW : zoom immédiat, sans indication à l'écran du niveau
  de zoom actuel.

**Frictions et ambiguïtés identifiées dans le code** (pas de test utilisateur réel
effectué, déduites de la lecture du comportement) :
- Aucune confirmation avant "Relancer" : la partie en cours est perdue sans avertissement.
- Aucun message d'erreur visible en jeu si un asset de texture est introuvable ou si
  Godot lui-même est absent (les erreurs correspondantes, côté `run_headless.py`/
  `run_screenshot.py`, ne s'affichent qu'en console, pas dans la fenêtre du jeu).
- Les sliders "Réglages" par personnage sont repliés par défaut : un utilisateur pressé
  peut ne jamais découvrir qu'ils existent.
- Aucune IA/LLM branché : le comportement "intelligent" observé (cueillette, mémoire)
  est en réalité un automate à seuils — l'interface ne le précise pas, ce qui peut
  laisser croire à tort à une vraie prise de décision par IA à ce stade du projet.
...
(campagnes de tests automatisées), `synthese-projet` (cette skill, génère ce document).
...
- UX non testée avec un utilisateur réel : les frictions listées en section 7 sont
  déduites de la lecture du code, pas d'une observation d'usage.

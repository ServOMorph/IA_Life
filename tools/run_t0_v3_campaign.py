"""Exécute les lignées T0 v3 dans des processus Godot isolés."""
...
from t0_v3_results import EXPERIMENT_ID, FINGERPRINT, INITIALIZATION_SEEDS, validate_records

GODOT_EXE = Path("D:/Godot/godot.exe")
PROJECT_DIR = Path(__file__).resolve().parent.parent
VALIDATION_CARDS = list(range(330000101, 330000133))
...
def expected_records() -> list[dict]:
    records: list[dict] = []
    for seed in sorted(INITIALIZATION_SEEDS):
        for arm in ("scripted", "initial_frozen", "random_valid"):
            records.extend(_expected_record(arm, seed, card_seed, 0) for card_seed in VALIDATION_CARDS)
        for arm in ("trained", "reset_each_episode"):
            for checkpoint in CHECKPOINTS:
                records.extend(_expected_record(arm, seed, card_seed, checkpoint) for card_seed in VALIDATION_CARDS)
    return records


def _expected_record(arm: str, initialization_seed: int, card_seed: int, checkpoint: int) -> dict:
    return {"experiment_id": EXPERIMENT_ID, "arm": arm, "initialization_seed": initialization_seed, "card_seed": card_seed, "checkpoint": checkpoint, "success": False, "terminated": False, "truncated": True, "table_checksum": "expected", "config_fingerprint": FINGERPRINT}


def worker_command(output: Path, kind: str, initialization_seed: int) -> list[str]:
    return [str(GODOT_EXE), "--headless", "--fixed-fps", "60", "--path", str(PROJECT_DIR), "--log-file", str(output.with_suffix(".godot.log")), "--scene", "tools/t0_v3_campaign.tscn", "--", "--output", str(output), "--kind", kind, "--initialization-seed", str(initialization_seed), "--contract", EXPERIMENT_ID]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Exécute les lignées T0 v3 en processus Godot isolés.")
    ...
    tasks = [(worker_dir / f"{kind}_{seed}.jsonl", kind, seed) for kind in KINDS for seed in sorted(INITIALIZATION_SEEDS)]
    if args.dry_run:
    ...
    print(f"SUCCÈS : {len(records)} résultats T0 v3 agrégés dans {output}.")
    return 0
